...
export const deleteFieldsInput = z.object({
  tableId: z.string(),
  fieldIds: z.array(z.string()).min(1).max(2000),
});
...
export const SafeId = z.string().regex(ID_REGEX, "ID must be alphanumeric with hyphens/underscores, max 64 chars");

// ── Trash ────────────────────────────────────────────────────────────────
export const getTableTrashInput = z.object({
  baseId: SafeId.describe("The Teable Base ID"),
});

export const restoreTableFromTrashInput = z.object({
  trashId: SafeId.describe("The trash item ID from get_table_trash results"),
});

export const permanentlyDeleteTableInput = z.object({
  baseId: SafeId.describe("The Teable Base ID"),
  tableId: SafeId.describe("The Teable Table ID to permanently delete"),
});

// ── Export ───────────────────────────────────────────────────────────────
export const exportTableDataInput = z.object({
  tableId: SafeId.describe("The Teable Table ID to export"),
});

// ── Share View ───────────────────────────────────────────────────────────
export const shareViewInput = z.object({
  tableId: SafeId.describe("The Teable Table ID"),
  viewId: SafeId.describe("The view ID to share/unshare"),
  enableShare: z.boolean().describe("true to enable sharing, false to disable"),
});

// ── Dependency Graph ─────────────────────────────────────────────────────
export const getFieldDependencyGraphInput = z.object({
  tableId: SafeId.describe("The Teable Table ID"),
});

export const analyzeFieldImpactInput = z.object({
  tableId: SafeId.describe("The Teable Table ID"),
  fieldId: SafeId.describe("The Field ID to analyze"),
});
